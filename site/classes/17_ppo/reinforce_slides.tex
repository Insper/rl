...
\begin{frame}{Algoritmo}
	\begin{algorithmic} 
		...
		\STATE gerar um episódio ${s_{0},a_{0},r_{0},\cdots,s_{T-1},a_{T-1},r_{T}}$ seguindo $\pi(.|.,\theta)$
		\FOR {cada passo dentro do episódio $t = 0, 1, 2, \cdots, T-1$}
			\STATE $G \leftarrow \sum_{t=0}^{T} \gamma^{t} \times r_{t}$
...
\begin{frame}
	\begin{algorithmic} 
		...
		\STATE inicializar os valores de $\theta$ para a policy $\pi(A|S,\theta)$ arbitrariamente
		\FOR {todos os episódios}
		\STATE \HiLi gerar um episódio ${s_{0},a_{0},r_{0},\cdots,s_{T-1},a_{T-1},r_{T}}$ seguindo $\pi(.|.,\theta)$
		\FOR {cada passo dentro do episódio $t = 0, 1, 2, \cdots, T-1$}
		\STATE $G \leftarrow \sum_{t=0}^{T} \gamma^{t} \times r_{t}$
		\STATE $\theta \leftarrow \theta + \alpha \times \bigtriangledown \log \pi(a_{t}|s_{t}, \theta) \times G$
		\ENDFOR
...
\begin{frame}
	\begin{algorithmic} 
		...
		\STATE gerar um episódio ${s_{0},a_{0},r_{0},\cdots,s_{T-1},a_{T-1},r_{T}}$ seguindo $\pi(.|.,\theta)$
		\FOR {cada passo dentro do episódio $t = 0, 1, 2, \cdots, T-1$}
		\STATE \HiLi $G \leftarrow \sum_{t=0}^{T} \gamma^{t} \times r_{t}$
		\STATE $\theta \leftarrow \theta + \alpha \times \bigtriangledown \log \pi(a_{t}|s_{t}, \theta) \times G$
		\ENDFOR
		\ENDFOR
	\end{algorithmic}
	...
	\begin{itemize}
		\item Se $T=3$ então $G = \gamma^{0}r_{0} + \gamma^{1}r_{1} + \gamma^{2}r_{2} + \gamma^{3}r_{3}$
		\item Muitos dos artigos sobre PO descrevem $G$ como \textit{discounted reward}.
		\item Também conhecido como \textit{reforço acumulado} em Q-Learning. 
	\end{itemize}
\end{frame}
