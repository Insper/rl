...
\begin{itemize}
	...
	\item $\hat{V}(s_{t}) = \sum_{a_{t} \in A} \pi (a_{t}|s_{t}).\hat{Q}(s_{t},a_{t})$ função baseline que é a média ponderada dos retornos de todas as ações possíveis a partir de $s_t$.
\end{itemize}	

\vspace{0.5cm}

Isto reduz a variância do gradiente porque centra o sinal do aprendizado em torno de zero. 
...
O algoritmo PPO define uma nova função objetivo que garante que a política nova não fique muito longe da política antiga:
	...
	\begin{equation}
	L^{CLIP}(\theta) = \hat{E} \left[ \nonumber \min \left( \nonumber \textcolor{blue}{r(\theta).\hat{A}(s_{t},a_{t})}, \textcolor{red}{clip}(\textcolor{green}{r(\theta)}, \textcolor{purple}{1 - \epsilon}, \textcolor{cyan}{1 + \epsilon})\textcolor{red}{.\hat{A}(s_{t},a_{t})} \nonumber \right) \right]		
	\end{equation}
...
\begin{frame}{Revisando o $L^{CLIP}$}

	\begin{equation}
	L^{CLIP}(\theta) = \hat{E} \left[ \min \left( \textcolor{blue}{r(\theta).\hat{A}(s_{t},a_{t})}, \textcolor{red}{clip(r(\theta), 1 - \epsilon, 1 + \epsilon).\hat{A}(s_{t},a_{t})} \right) \right]
	\end{equation}
	...
	\begin{itemize}
		\item $r(\theta)$: razão entre a política atual e a política antiga.
		\item $\hat{A}(s_{t},a_{t})$: estimador de vantagem, quão melhor ou pior a ação $a_t$ foi em relação à média das ações possíveis no estado $s_t$.
	\end{itemize}
...
\begin{itemize}
	\item \textcolor{blue}{atores}: responsáveis por gerar os episódios e calcular toda a sequência de $s_t, a_t, r_{t+1}, \hat{A}_{t+1}, \cdots$ com base na policy vigente.
	\item \textcolor{red}{críticos}: responsáveis por otimizar os valores da policy. 
\end{itemize}
...
\begin{frame}{Algoritmo PPO, estilo "Actor-Critic"}
	...
	\begin{itemize}
		...
		\item executar treinamentos usando minibatch e com $K$ épocas não é usual em \textit{Reinforcement Learning}. No entanto, devido ao uso da função $L^{CLIP}$ isso é possível.  
	\end{itemize}
