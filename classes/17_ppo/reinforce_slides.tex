...
\begin{frame}
	\begin{algorithmic} 
		...
		\STATE gerar um episódio ${s_{0},a_{0},r_{1},\cdots,s_{T-1},a_{T-1},r_{T}}$ seguindo $\pi(.|.,\theta)$
		\FOR {cada passo dentro do episódio $t = 0, 1, 2, \cdots, T-1$}
		\STATE \HiLi $G \leftarrow \sum_{t=1}^{T} \gamma^{t} \times r_{t}$
		\STATE $\theta \leftarrow \theta + \alpha \times \bigtriangledown \log \pi(a_{t}|s_{t}, \theta) \times G$
		\ENDFOR
		\ENDFOR
	\end{algorithmic}
	...
	\begin{itemize}
		\item Isto já conhecemos. Conhecemos como reforço acumulado. 
		\item Muitos dos artigos sobre PO descrevem isto como \textit{discounted reward}. 
	\end{itemize}
\end{frame}
